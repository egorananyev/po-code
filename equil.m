...
% Displaying the threshold values
display(strcat('threshMask=',mat2str(threshMask)));
display(strcat('threshNoMask=',mat2str(threshNoMask)));
...
disp.boxSize = 90;
disp.distX = 180; % 150; % display center distance from the vertical midline (left/right)
...
logStep = c2n(condTable,'logStep',1); % assuming it's the same for all staircs
for i=1:(numofConds-1)
    if logStep
        step = (log10(threshMask)-log10(0.01))/3;
        startVal = log10(threshMask)+step*(randi(6)-5); % stairc start random
        xMinVal = log10(0.001); xMaxVal = log10(2);
    else
        step = (threshMask-.01)/3; % the step size is determined by the range
        startVal = threshMask+step*(randi(6)-5);
        xMinVal = 0; xMaxVal = 2;
    end
    staircs(i) = PAL_AMUD_setupUD('Up',c2n(condTable,'stairUp',i),...
        ...
        'startValue',startVal,'xMin',xMinVal,'xMax',xMaxVal); %#ok<AGROW>
end
display(strcat('step=',mat2str(step)));
...
for blockTrial=1:numofStaircs
    ...
    respMadeCont = false;
    blankFdbk = true; % set this to true when disabling blank feedback
    falsePos = false;
    ...
    if d.staircVal(c) <= 1 % if the staircase value is below 1, mask is at full contr
        d.maskContr(c) = 1;
        if logStep
            d.maxContr(c) = 10^d.staircVal(c);
        else
            d.maxContr(c) = d.staircVal(c);
        end
    else
        if logStep
            d.maskContr(c) = 2-10^d.staircVal(c);
        else
    ...
    while ~respMadeCont
        ...
        if curFrame>d.trialFC(c) && ~falsePos
        ...
        %% Blank trial feedback: % Disabled as of 2015-11-27
        if 1>2 % respMadeOdt && respMadeSubjVis && ~blankFdbk
            if d.respSubjVis(c)>1 && d.curStairc(c)==numofConds && ~falsePos
                endFrame = curFrame + feedbackFC ;
                falsePos = true;
                display('False positive response made! Providing feedback.');
            end
            if falsePos
                if curFrame < endFrame
                    % Displaying the "continue" text:
                    Screen('TextSize', wPtr, 25);
                    DrawFormattedText(wPtr, 'Blank trial\nFalse positive','center','center', ...
                        [255 255 50], 14, [], [], [], [], boxL);
                    DrawFormattedText(wPtr, 'Blank trial\nFalse positive','center','center', ...
                        [255 255 50], 14, [], [], [], [], boxR);
                else
        ...
        if respMadeOdt && respMadeSubjVis && ~respMadeCont && blankFdbk
            % Displaying the "continue" text:
